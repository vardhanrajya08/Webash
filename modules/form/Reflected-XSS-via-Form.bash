...
function XSSFormBased() {
	...
	do
		TEMPDIR="$(cd "$(dirname "$0")/../../webash-tempdata" ; pwd -P)"
		...
		then
			...
			then
				...
				PARAMSNOTNULL=$(echo "${PARAMS}" | sed 's/=\([a-zA-Z0-9]*\)$/=\1Webash%22XSS/g' | sed 's/=\([a-zA-Z0-9]*\)\&/=\1Webash%22XSS\&/g')
				ATTACK_1="${GETURL}?${PARAMS}"
				ATTACK_2="${GETURL}?${PARAMSNOTNULL}"
				if [[ $(curl -skL --connect-timeout 10 --max-time 10 -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" "${ATTACK_1}" | grep 'Webash"XSS') ]]
				then
					PushAlert "${GETURL}" "GET" "${PARAMS}"
				elif [[ $(curl -skL --connect-timeout 10 --max-time 10 -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" "${ATTACK_2}" | grep 'Webash"XSS') ]]; then
		...
		then
			...
			then
				...
				ATTACK_1="${GETURL}"
				ATTACK_2="${GETURL}"
				if [[ ! -z $(curl -skL --connect-timeout 10 --max-time 10 -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -X POST "${ATTACK_1}" --data "${PARAMS}" | grep 'Webash"XSS') ]]
				then
					PushAlert "${GETURL}" "POST" "${PARAMS}"
				elif [[ ! -z $(curl -skL --connect-timeout 10 --max-time 10 -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -X POST "${ATTACK_2}" --data "${PARAMSNOTNULL}" | grep 'Webash"XSS') ]]; then
